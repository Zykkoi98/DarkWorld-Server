...
// ===== ЧАСТЬ 1 ИЗ 2: НАДЕТЬ ПРЕДМЕТ С УЧЕТОМ СТАКОВ ИДЕНТИФИКАТОРОВ =====
// ============================================================================
...
module.exports = function(io, socket, sb) {
  ...
  socket.on('equip_item_secure', async ({ userId, itemId: itemUuidOrId }) => {
    try {
      ...
      // ШАГ 1: Извлекаем чистый ID предмета
      let cleanItemId = itemUuidOrId;
      if (itemUuidOrId && itemUuidOrId.includes('_') && !ITEMS_STAT_DB[itemUuidOrId] && !SERVER_CONSUMABLES_SLOTS[itemUuidOrId]) {
      ...
      // 🔥 ФИКС ИНДЕКСА: Расходники ищем по жесткому ID, а вещи — по UUID!
  ...
  socket.on('unequip_item_secure', async ({ userId, slotKey, ringIndex }) => {
    try {
      ...
      // 🛡️ Проверяем лимит сумки для шмоток (расходники стакаются, на них лимит не влияет)
      if (invTab === 'equipment' && inventory.equipment.length >= 30) {
        return socket.emit('error', '⚠️ Сумка переполнена! Некуда снять вещь.');
      }

      // 🔥 ФИКС ВОЗВРАТА СТАКА: Расходники честно складываем по countToReturn!
      ...
      // Для обычного снаряжения генерируем легальный UUID
      else {
